...
public void hexMultiplication() {
    ...
    if (parts.length == 2) {
        ...
        if (isValidHexadecimalWithDecimal(hex1) && isValidHexadecimalWithDecimal(hex2)) {
            String resultHex = performHexadecimalMultiplication(hex1, hex2);
...
private String performHexadecimalMultiplication(String hex1, String hex2) {
    double decimal1 = hexToDecimalWithDecimal(hex1);
    double decimal2 = hexToDecimalWithDecimal(hex2);
    double product = decimal1 * decimal2;

    // Convert the product back to hexadecimal with uppercase letters and no trailing zeros
    String resultHex = decimalToHexWithDecimal(product);

    return resultHex;
}

// The rest of the code, including isValidHexadecimalWithDecimal, hexToDecimalWithDecimal, and decimalToHexWithDecimal, remains the same.
...
private String decimalToHexWithDecimal(double decimal) {
    String[] parts = String.format("%.8f", decimal).split("\\.");
    int integerPart = Integer.parseInt(parts[0]);
    String hexIntegerPart = Integer.toHexString(integerPart).toUpperCase();
    String hexFractionalPart = "";

    if (parts.length > 1) {
        String fractionalPart = parts[1].substring(0, Math.min(8, parts[1].length())); // Limit to 8 decimal places
